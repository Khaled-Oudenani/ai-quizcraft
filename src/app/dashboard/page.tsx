...
export default async function DashboardPage() {
  ...
  const totalAttempts =
    quizzes?.reduce((sum, q) => sum + (q.attempts_count || 0), 0) || 0;
  const avgScore =
    quizzes
      ?.filter((q) => q.avg_score)
      .reduce((sum, q, _, arr) => sum + q.avg_score / arr.length, 0) || 0;
  const firstName =
  ...
  const stats = [
    ...
    },
    {
      ...
      icon: "public",
      color: "#45dfa4",
      change: `${totalQuizzes - publishedQuizzes} drafts`,
    },
    {
      label: "Attempts",
      value: totalAttempts,
  ...
  return (
    <div className="max-w-[1400px] mx-auto p-4 sm:p-6 lg:p-10">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          <p
            className="text-[#8c909e] text-xs sm:text-sm font-medium mb-1"
            style={{ fontFamily: "Manrope, sans-serif" }}
          >
        ...
        <Link
          href="/quiz/create"
          className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold text-[#00285b] text-sm transition-all hover:brightness-110 whitespace-nowrap shrink-0"
          style={{
            background: "linear-gradient(to bottom, #acc7ff, #508ff8)",
            fontFamily: "Manrope, sans-serif",
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-3 sm:gap-4 mb-8">
        {stats.map((stat) => (
          <div
            key={stat.label}
            className="rounded-2xl p-4 sm:p-5 flex flex-col gap-3 border border-[#424753]/20"
            style={{ background: "#181c24" }}
          >
            <div
            ...
            <div>
              <div
                className="text-2xl sm:text-3xl font-black text-[#dfe2ee] mb-0.5"
                style={{ fontFamily: "Space Grotesk, sans-serif" }}
              >
                {stat.value}
              </div>
              <div
                className="text-[10px] sm:text-xs font-bold text-[#8c909e] uppercase tracking-wider"
                style={{ fontFamily: "Manrope, sans-serif" }}
              >
                {stat.label}
              </div>
              <div
                className="text-[10px] sm:text-xs text-[#424753] mt-1 hidden sm:block"
                style={{ fontFamily: "Manrope, sans-serif" }}
              >
                {stat.change}
              </div>
      ...
      <div className="mb-5 flex justify-between items-center">
        <h2
          className="text-lg sm:text-xl font-bold text-[#dfe2ee]"
          style={{ fontFamily: "Space Grotesk, sans-serif" }}
        >
          Recent Quizzes
        </h2>
        <Link
          ...
          className="text-xs sm:text-sm text-[#acc7ff] hover:text-[#dfe2ee] transition-colors flex items-center gap-1"
          style={{ fontFamily: "Manrope, sans-serif" }}
        >
          View all
...
function QuizCard({ quiz }: { quiz: Quiz }) {
  ...
  return (
    <div
      className="rounded-2xl p-4 sm:p-5 flex flex-col gap-3 sm:gap-4 border border-[#424753]/20 hover:border-[#424753]/40 transition-all group"
      style={{ background: "#181c24" }}
    >
      <div className="flex justify-between items-start">
        ...
        <Link
          href={`/quiz/${quiz.id}/edit`}
          className="opacity-0 group-hover:opacity-100 w-7 h-7 rounded-lg bg-white/5 hover:bg-white/10 flex items-center justify-center transition-all shrink-0"
        >
      ...
      <div className="flex-1">
        <h3
          className="font-bold text-[#dfe2ee] mb-1 line-clamp-2 leading-tight text-sm sm:text-base"
          style={{ fontFamily: "Space Grotesk, sans-serif" }}
        >
          {quiz.title}
        </h3>
      ...
      <div className="flex items-center justify-between pt-3 border-t border-white/5">
        <div className="flex items-center gap-3">
          <span
            className="flex items-center gap-1 text-xs text-[#8c909e]"
            style={{ fontFamily: "Manrope, sans-serif" }}
          >
            <span className="material-symbols-outlined text-xs">quiz</span>
            {quiz.questions_count} Qs
...
function CreateNewCard() {
  return (
    <Link
      href="/quiz/create"
      className="rounded-2xl p-5 flex flex-col items-center justify-center gap-3 border border-dashed border-[#424753]/40 hover:border-[#acc7ff]/40 hover:bg-[#acc7ff]/5 transition-all group min-h-[180px] sm:min-h-[200px]"
    >
      <div className="w-10 h-10 rounded-xl bg-[#acc7ff]/10 group-hover:bg-[#acc7ff]/20 flex items-center justify-center transition-all">
        <span className="material-symbols-outlined text-[#acc7ff]">add</span>
      </div>
      <div className="text-center">
        <p
          className="font-bold text-[#8c909e] group-hover:text-[#acc7ff] transition-colors text-sm"
          style={{ fontFamily: "Space Grotesk, sans-serif" }}
        >
          Create New Quiz
        </p>
        <p
          className="text-xs text-[#424753] mt-1"
          style={{ fontFamily: "Manrope, sans-serif" }}
        >
          AI-powered generation
        </p>
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center py-16 sm:py-20 text-center">
      <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl bg-[#acc7ff]/10 flex items-center justify-center mb-5">
        <span className="material-symbols-outlined text-2xl sm:text-3xl text-[#acc7ff]">
          psychology
        </span>
      </div>
      <h3
        className="text-lg sm:text-xl font-bold text-[#dfe2ee] mb-2"
        style={{ fontFamily: "Space Grotesk, sans-serif" }}
      >
        No quizzes yet
      </h3>
      <p
        className="text-sm text-[#8c909e] max-w-xs mb-6"
        style={{ fontFamily: "Manrope, sans-serif" }}
      >
        Upload a document and let AI generate your first quiz in under 60
        seconds.
      </p>
      <Link
        href="/quiz/create"
