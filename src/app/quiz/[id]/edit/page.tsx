...
export default function QuizEditPage() {
  ...
  // Sync mobile view with active question
  useEffect(() => {
    if (activeQuestion) {
  ...
  async function updateQuestion(qId: string, updates: Partial<Question>) {
    ...
    if (updates.correct_answer)
      dbUpdates.correct_answer = JSON.stringify(updates.correct_answer);
    await supabase.from("questions").update(dbUpdates).eq("id", qId);
  }
  ...
  if (loading) {
    return (
      <div className="flex h-screen" style={{ background: "#0f131c" }}>
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="flex flex-col items-center gap-3">
            <span className="material-symbols-outlined text-4xl text-[#acc7ff] animate-spin">
              refresh
            </span>
            <p
    ...
    >
      ...
      {sidebarOpen && (
        <div className="lg:hidden fixed inset-0 z-50">
          ...
          />
          <div className="relative z-10 h-full w-72 animate-slide-in-left">
            <Sidebar />
          </div>
        </div>
      )}
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* Mobile Header */}
        <div
          className="lg:hidden flex items-center gap-3 px-4 py-3 border-b border-white/5 shrink-0"
          style={{ background: "#0a0e16" }}
        >
          {/* Hamburger */}
          <button
          ...
          >
            <span className="material-symbols-outlined text-xl text-[#8c909e]">
              menu
            </span>
          </button>
          ...
          <div className="flex-1 min-w-0">
            {mobileView === "editor" ? (
              <button
                ...
                className="flex items-center gap-1.5 text-sm text-[#acc7ff] font-medium"
                style={{ fontFamily: "Manrope, sans-serif" }}
              >
                <span className="material-symbols-outlined text-lg">
                  arrow_back
                </span>
                <span className="truncate">Back to questions</span>
              </button>
            ) : (
              <div>
                <h2
                  className="text-sm font-bold text-[#dfe2ee] truncate"
                  style={{ fontFamily: "Space Grotesk, sans-serif" }}
                >
                  {quiz.title}
                </h2>
                <p
                  className="text-[11px] text-[#8c909e]"
                  style={{ fontFamily: "Manrope, sans-serif" }}
                >
                  {questions.length} questions
                </p>
          ...
          <span
            ...
            style={{
              background: quiz.status === "published" ? "#45dfa420" : "#262a33",
              color: quiz.status === "published" ? "#45dfa4" : "#8c909e",
              fontFamily: "Manrope, sans-serif",
            }}
          >
        ...
        {/* Content Area */}
        <div className="flex-1 flex overflow-hidden">
          {/* Question List Panel — Mobile: full when list view, Desktop: sidebar width */}
          <div
            className={`
              flex flex-col border-r border-white/5 overflow-y-auto shrink-0
          ...
          >
            ...
            <div className="hidden lg:block p-5 border-b border-white/5">
              <div className="flex justify-between items-start gap-2 mb-4">
                <div className="flex-1 min-w-0">
                  ...
                  <p
                    className="text-xs text-[#8c909e] mt-0.5"
                    style={{ fontFamily: "Manrope, sans-serif" }}
                  >
                    {questions.length} questions
                  </p>
            ...
            {/* Mobile: Publish + Quick actions combined */}
            <div className="lg:hidden p-3 border-b border-white/5 flex flex-col gap-2">
              <button
                onClick={handlePublish}
                disabled={saving || quiz.status === "published"}
                className="w-full py-2.5 rounded-xl text-sm font-bold transition-all hover:brightness-110 disabled:opacity-50 flex items-center justify-center gap-2"
            ...
            <div className="flex gap-1 p-3 border-b border-white/5">
              {[
                ...
                >
                  ...
                  <span
                    className="text-[10px] text-[#424753]"
                    style={{ fontFamily: "Manrope, sans-serif" }}
                  >
                    {action.label}
                  </span>
                  ...
                  style={{
                    background:
                      activeQuestion === q.id
                        ...
                        : "#181c24",
                    borderColor:
                      activeQuestion === q.id ? "#acc7ff40" : "#42475340",
                  }}
                >
                  <div className="flex items-start gap-2.5">
                    <span
                      ...
                      style={{
                        background:
                          activeQuestion === q.id ? "#acc7ff" : "#262a33",
                        color: activeQuestion === q.id ? "#001a40" : "#8c909e",
                        fontFamily: "Manrope, sans-serif",
                      }}
                    >
          ...
          <div
            className={`
              flex-1 overflow-y-auto neural-mesh
              ${mobileView === "editor" ? "flex" : "hidden"}
              lg:flex
              ${mobileView === "editor" ? "flex-col" : ""}
            `}
            style={{ background: "#0f131c" }}
          >
            {activeQ ? (
              <div className="max-w-2xl mx-auto w-full p-4 sm:p-6 lg:p-8">
                {/* Editor header — Mobile back shown in top bar, Desktop inline */}
                <div className="hidden lg:flex justify-between items-center mb-6">
                  <h3
                    className="font-bold text-[#dfe2ee]"
                    style={{ fontFamily: "Space Grotesk, sans-serif" }}
                  >
                    Edit Question{" "}
                ...
                <div className="flex flex-col gap-5">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      rows={3}
                      className="w-full px-3 sm:px-4 py-3 rounded-xl text-sm text-[#dfe2ee] placeholder-[#424753] outline-none resize-none transition-all"
                      style={{
                        background: "#181c24",
                  ...
                  {activeQ.options && (
                    <div>
                      <label
                        className="block text-xs font-bold text-[#c2c6d5] mb-3 uppercase tracking-wider"
                        style={{ fontFamily: "Manrope, sans-serif" }}
                      >
                        Answer Options
                      </label>
                        ...
                        {(activeQ.options as string[]).map((option, idx) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              >
                                {isCorrect && (
                                  <span className="material-symbols-outlined text-[10px] text-[#003825]">
                                    check
                                  </span>
                                )}
                              </button>
                              <input
                                ...
                                onChange={(e) => {
                                  const newOptions = [
                                    ...(activeQ.options as string[]),
                                  ];
                                  const wasCorrect =
                                    activeQ.correct_answer === option;
                                  newOptions[idx] = e.target.value;
                                  updateQuestion(activeQ.id, {
                                    options: newOptions,
                                    ...(wasCorrect
                                ...
                                className="flex-1 px-3 py-2.5 rounded-xl text-sm text-[#dfe2ee] outline-none transition-all min-w-0"
                                style={{
                                  background: isCorrect
                                    ? "#45dfa415"
                                    : "#181c24",
                                  border: `1px solid ${
                                    isCorrect ? "#45dfa440" : "#424753"
                                  }`,
                                  fontFamily: "Manrope, sans-serif",
                                }}
                              />
                      ...
                      <p
                        className="text-xs text-[#8c909e] mt-2"
                        style={{ fontFamily: "Manrope, sans-serif" }}
                      >
                  ...
                  {activeQ.type === "short_answer" && (
                    <div>
                      ...
                      <textarea
                        ...
                        rows={2}
                        className="w-full px-3 sm:px-4 py-3 rounded-xl text-sm text-[#dfe2ee] outline-none resize-none transition-all"
                        style={{
                          background: "#181c24",
                          border: "1px solid #424753",
                          fontFamily: "Manrope, sans-serif",
                  ...
                  <div>
                    ...
                    <input
                      ...
                      min={1}
                      max={10}
                      className="w-24 px-4 py-3 rounded-xl text-sm text-[#dfe2ee] outline-none transition-all"
                      style={{
                        background: "#181c24",
                        border: "1px solid #424753",
                        fontFamily: "Manrope, sans-serif",
              ...
              <div className="hidden lg:flex flex-col items-center justify-center h-full gap-4 text-center p-8">
                ...
                <div>
                  ...
                  <p
                    className="text-sm text-[#8c909e] mt-1"
                    style={{ fontFamily: "Manrope, sans-serif" }}
                  >
                    Click any question from the left panel
                  </p>
                </div>
              </div>
            )}
      ...
      {/* Inline style for sidebar slide-in animation */}
      <style jsx global>{`
