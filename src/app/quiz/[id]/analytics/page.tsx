...
export default function AnalyticsPage() {
  ...
  const passRate =
    attempts.length > 0
      ? Math.round(
          (attempts.filter((a) => a.percentage >= (quiz?.pass_mark || 60))
            .length /
            attempts.length) *
            100,
        )
      : 0;
  const avgTime =
    attempts.length > 0 && attempts.some((a) => a.time_taken)
      ? Math.round(
          attempts
            .filter((a) => a.time_taken)
            .reduce((sum, a) => sum + (a.time_taken || 0), 0) /
            attempts.filter((a) => a.time_taken).length,
        )
      : null;
  ...
  const distribution = [
    ...
    {
      range: "20-40%",
      count: attempts.filter((a) => a.percentage >= 20 && a.percentage < 40)
        .length,
    },
    {
      range: "40-60%",
      count: attempts.filter((a) => a.percentage >= 40 && a.percentage < 60)
        .length,
    },
    {
      range: "60-80%",
      count: attempts.filter((a) => a.percentage >= 60 && a.percentage < 80)
        .length,
    },
    {
      range: "80-100%",
      count: attempts.filter((a) => a.percentage >= 80).length,
    },
  ];

  // Score over time (last 20)
  const scoreTimeline = attempts
    .slice(0, 20)
    .reverse()
    .map((a, i) => ({
      attempt: i + 1,
  ...
  const customTooltip = ({
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div
          className="px-3 py-2 rounded-lg border border-[#424753]/40 text-xs"
          style={{ background: "#262a33", fontFamily: "Manrope, sans-serif" }}
        >
          <p className="text-[#8c909e]">{label}</p>
          <p className="text-[#acc7ff] font-bold">
            {val}
            {typeof val === "number" &&
            val <= 100 &&
            label !== distribution[0]?.range
              ? "%"
              : ""}
          </p>
        </div>
  ...
  return (
    ...
    >
      <Sidebar />
      <main
        className="flex-1 overflow-y-auto neural-mesh p-8 lg:p-10"
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex justify-between items-start mb-8">
            <div>
              <Link
                href="/dashboard"
                className="text-xs text-[#8c909e] hover:text-[#acc7ff] flex items-center gap-1 mb-2"
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            {[
              ...
              <div
                key={stat.label}
                className="rounded-2xl p-5 border border-[#424753]/20"
              ...
              >
                ...
                <p
                  className="text-xs text-[#8c909e] mt-0.5 uppercase tracking-wider"
                  style={{ fontFamily: "Manrope, sans-serif" }}
                >
                  {stat.label}
                </p>
          ...
          {attempts.length === 0 ? (
            <div
              className="flex flex-col items-center justify-center py-20 text-center rounded-2xl border border-[#424753]/20"
              style={{ background: "#181c24" }}
            >
              <div className="w-14 h-14 rounded-2xl bg-[#acc7ff]/10 flex items-center justify-center mb-4">
              ...
              <p
                ...
                style={{ fontFamily: "Manrope, sans-serif" }}
              >
                Share your quiz to start collecting responses and see analytics
                here.
              </p>
              <Link
                href={`/quiz/${quizId}/share`}
                className="px-4 py-2.5 rounded-xl text-sm font-bold text-[#00285b]"
                style={{
                  background: "linear-gradient(to bottom, #acc7ff, #508ff8)",
                  fontFamily: "Manrope, sans-serif",
                }}
            ...
            <div className="grid lg:grid-cols-2 gap-6 mb-6">
              {/* Score Distribution */}
              <div
                className="rounded-2xl p-6 border border-[#424753]/20"
                style={{ background: "#181c24" }}
              >
                <h3
                  className="font-bold text-[#dfe2ee] mb-5"
                  style={{ fontFamily: "Space Grotesk, sans-serif" }}
                >
                  Score Distribution
                </h3>
                <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={distribution} barSize={28}>
                    ...
                    <Tooltip
                      ...
                      cursor={{ fill: "rgba(172,199,255,0.05)" }}
                    />
                    <Bar dataKey="count" fill="#508ff8" radius={[6, 6, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
                  ...
                  <thead>
                    <tr style={{ background: "#0f131c" }}>
                      {["Student", "Score", "Time", "Date"].map((h) => (
                        <th
                          key={h}
                          className="px-6 py-3 text-left text-xs font-bold text-[#8c909e] uppercase tracking-wider"
                          style={{ fontFamily: "Manrope, sans-serif" }}
                        >
                          {h}
                        </th>
                    ...
                    {attempts.slice(0, 10).map((attempt) => (
                      ...
                      >
                        <td className="px-6 py-4">
                          <div>
                            <p
                              className="text-sm font-medium text-[#dfe2ee]"
                              style={{ fontFamily: "Manrope, sans-serif" }}
                            >
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            <span
                              ...
                              style={{
                                color:
                                  attempt.percentage >= (quiz?.pass_mark || 60)
                                    ? "#45dfa4"
                                    : "#ffb4ab",
                                fontFamily: "Space Grotesk, sans-serif",
                              }}
